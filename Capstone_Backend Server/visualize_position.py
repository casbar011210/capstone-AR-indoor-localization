...
def clear_all():
    ...
    for f in [HERE/"outputs"/"features.h5",
...
def draw(ax, sfm_pos, records):
    ...
    if not records:
        ...
        if hx:
            ...
            ax.plot(hx + [-records[-1]["camera_position"][0]],
                    hy + [ records[-1]["camera_position"][1]],
                    c="#ff6b6b", lw=0.8, alpha=0.3)
        ...
        ax.annotate("", xy=(px + fwd_x*arrow_len, py + fwd_y*arrow_len),
                    xytext=(px, py),
                    arrowprops=dict(arrowstyle="->", color="#ff3366", lw=2))

        ax.set_title(f"Top-down view (XY)  |  pos: ({px:.3f}, {py:.3f})  |  records: {len(records)}",
                     color="white", fontsize=12)

    margin = max(xs.ptp(), ys.ptp()) * 0.25
    ax.set_xlim(xs.min()-margin, xs.max()+margin)
    ax.set_ylim(ys.min()-margin, ys.max()+margin)
    ax.set_xlabel("X (mirrored)", color="gray")
